...
class Pipeline:
    ...
    def __init__(self, channel, numKernels, kernelSize, dtype) -> None:
        """
        This is the initialization function for a pipeline managing input from one channel in a convolutional
        neural network layer.

        @param channel The number of the channel, which this pipeline is receiving spikes from.
    ...
    def applyConv(self, recSpike, neuronInLog = None, ln = None) -> ArrayLike:
        ...
        and one channel (specified by the pipeline) of each kernel. Each kernel will then apply the
